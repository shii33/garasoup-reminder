(()=>{
  ...
  function moodText(s,L){if(Date.now()<Number(s?.sleepUntil||0))return'🌙 すやすや寝てる';if(Number(s?.poop||0)>0)return'🚽 トイレしたみたい';const [k,v]=Object.entries(L).sort((a,b)=>a[1]-b[1])[0]||['mood',100];if(v>=55)return'☀️ ごきげんだよ';return{k:'',hunger:'🍚 おなかすいてる',sleep:'🌙 ねむそう',mood:'🎮 あそびたそう',clean:'🫧 そろそろおふろ'}[k]||'☀️ ごきげんだよ'}
  ...
  async function shareCurrent(){
    ...
    const pink=t==='み',accent=pink?'#f6c8d8':'#c9e3f7',room=pink?'#fff0f4':'#eef7ff',L=levels(s),[stageLabel,goal]=stage(s.growthXP),age=Math.max(0,Math.floor((Date.now()-new Date((s.born||new Date().toISOString().slice(0,10))+'T00:00:00').getTime())/864e5)),speech=document.getElementById('g11speech')?.textContent||s.lastSpeech||'……',mood=moodText(s,L),habit=Object.entries(s.learned||{}).filter(([,n])=>Number(n)>=3).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([w])=>w),trait=TM[topTrait(s)]||'まだクセ薄め';
    ...
    const cx=540,cy=475; x.fillStyle=accent;x.strokeStyle='#111';x.lineWidth=5;
    if(stageLabel==='たまご'){x.beginPath();x.ellipse(cx,cy+20,92,120,0,0,Math.PI*2);x.fill();x.stroke()}else{round(x,cx-86,cy-58,172,160,72);x.fill();x.stroke();x.beginPath();x.ellipse(cx-57,cy-52,36,48,-.35,0,Math.PI*2);x.fill();x.stroke();x.beginPath();x.ellipse(cx+57,cy-52,36,48,.35,0,Math.PI*2);x.fill();x.stroke();x.fillStyle='#111';if(Date.now()<Number(s.sleepUntil||0)){x.fillRect(cx-46,cy-4,25,5);x.fillRect(cx+21,cy-4,25,5)}else{x.beginPath();x.ellipse(cx-34,cy-4,10,14,0,0,Math.PI*2);x.fill();x.beginPath();x.ellipse(cx+34,cy-4,10,14,0,0,Math.PI*2);x.fill()}x.beginPath();x.arc(cx,cy+37,18,0,Math.PI);x.stroke()}
    round(x,310,312,460,80,15);x.fillStyle='#fff';x.fill();x.strokeStyle='#111';x.lineWidth=2;x.stroke();x.fillStyle='#111';x.font='800 23px sans-serif';x.textAlign='center';wrap(x,speech,cx,345,410,28,2);x.textAlign='left';
  ...
  function bind(){patchLabel();const old=document.getElementById('g11s');if(!old||old.dataset.currentShare==='1')return;const b=old.cloneNode(true);b.dataset.currentShare='1';old.replaceWith(b);b.addEventListener('click',async()=>{b.disabled=true;try{await shareCurrent()}finally{b.disabled=false}})}
