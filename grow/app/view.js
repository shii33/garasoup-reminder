...
export class GrowView{
  ...
  showWordResult(result){this.activity={type:'word-result',item:result?.item};this.openModal(`<div class="g12modalhead"><div><b>できた。</b><small>ひろいものに入った。</small></div><button data-modal-close>×</button></div><div class="g12bigphrase">${esc(result?.phrase||'')}</div><div class="g12modalactions"><button data-item-charm="${esc(result?.item?.id||'')}">この子につかう</button><button data-play-mode="words">もう1個つくる</button><button data-modal-close>あとで</button></div>`)}
  ...
  showFindItem(item,{gift=false}={}){if(!item)return;this.activity={type:'find',item};const text=item.payload?.text||item.payload?.word||'',action=item.type==='quiz'?`<button data-item-quiz="${esc(item.id)}">続きあてにつかう</button>`:['charm','crafted'].includes(item.type)?`<button data-item-charm="${esc(item.id)}">この子につかう</button>`:`<button data-item-word="${esc(item.id)}">ことば遊びにつかう</button>`;this.openModal(`<div class="g12modalhead"><div><b>${gift?'おかえり。':`${esc(item.icon||'•')} ${esc(item.title||'')}`}</b><small>${gift?'なんか持ってきた。':esc(item.subtitle||'')}</small></div><button data-modal-close>×</button></div>${gift?`<div class="g12gift"><span>${esc(item.icon||'🎁')}</span><b>${esc(item.title||'おみやげ')}</b><small>${esc(item.subtitle||'')}</small></div>`:text?`<div class="g12memoryquote">${esc(text)}</div>`:''}<div class="g12modalactions">${action}<button data-item-drop="${esc(item.id)}">手放す</button><button data-modal-close>あとで</button></div>`)}
  showCharmResult(r){this.activity={type:'charm-result'};this.openModal(`<div class="g12modalhead"><div><b>メロがれた。</b><small>つかったものは、ひろいものからなくなった。</small></div><button data-modal-close>×</button></div><div class="g12bigphrase">${esc(r?.item?.payload?.text||r?.item?.payload?.word||r?.item?.title||'')}</div><div class="g12modalactions"><button data-modal-close>おけ</button></div>`)}
