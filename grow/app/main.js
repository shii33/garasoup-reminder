import './data-service.js?v=20260926-refactor-10';
import {GrowModel,getViewer} from './core.js?v=20260926-refactor-10';
import {SourceLog} from './source-log.js?v=20260926-refactor-10';
import {GrowView} from './view.js?v=20260926-refactor-10';
import {ShareService} from './share.js?v=20260926-refactor-10';
...
const scheduleIdle=(min=6500,max=15000)=>{
  clearTimeout(idleTimer);
  idleTimer=setTimeout(async()=>{
    const poseChanged=!!view?.showIdlePose();
    const speechChanged=!!(await model?.idleSpeech());
    if(speechChanged)await view.render({preservePet:true});
    if(poseChanged||speechChanged)share.schedule(350);
    scheduleIdle();
  },rand(min,max));
};
const afterInteraction=()=>scheduleIdle(6500,11000);
